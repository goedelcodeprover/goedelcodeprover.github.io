...
const LEGEND = [
  { type: 'keyword',  label: '关键字',   example: 'theorem def' },
  { type: 'tactic',   label: '证明策略', example: 'simp rw induction' },
  { type: 'type',     label: '类型',     example: 'Nat Prop List' },
  { type: 'operator', label: '运算符',   example: '∧ ∨ → ∀ ∃' },
  { type: 'string',   label: '字符串',   example: '"hello"' },
  { type: 'number',   label: '数字',     example: '0 1 42' },
  { type: 'comment',  label: '注释',     example: '-- comment' },
  { type: 'ident',    label: '标识符',   example: 'fibonacci xs' },
];
  ...
  return (
    <div style={{
      ...
      <div style={{ marginTop: '32px', color: '#3d4451', fontSize: '11px', letterSpacing: '0.04em' }}>
        基于 Prism.js 语法规则 · 用于 Lean 4 定理证明语言
      </div>
    </div>
  );
}
